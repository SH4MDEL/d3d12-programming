...
CShader::~CShader()
{
	...
	{
		for (int i = 0; i < m_nPipelineStates; i++) m_ppd3dPipelineStates[i]->Release();
...
void CPlayerShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dRootSignature, UINT nRenderTargets, DXGI_FORMAT *pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat, int nPipelineState)
{
	m_nPipelineStates = 1;
...
{
	if (nPipelineState == 0) return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "CSHorizontalBlur", "cs_5_1", ppd3dShaderBlob));
	if (nPipelineState == 1) return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "CSVerticalBlur", "cs_5_1", ppd3dShaderBlob));
}
...
void CGaussian1DBlurComputeShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dRootSignature, UINT cxThreadGroups, UINT cyThreadGroups, UINT czThreadGroups, int nPipelineState)
{
	m_nPipelineStates = 2;
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];

	CComputeShader::CreateShader(pd3dDevice, pd3dRootSignature, cxThreadGroups, cyThreadGroups, czThreadGroups, 0);
	CComputeShader::CreateShader(pd3dDevice, pd3dRootSignature, cxThreadGroups, cyThreadGroups, czThreadGroups, 1);
