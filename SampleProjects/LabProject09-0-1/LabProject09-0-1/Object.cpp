...
CTexture::CTexture(int nTextures, UINT nTextureType, int nSamplers, int nGraphicsSrvRootParameters, int nComputeUavRootParameters, int nComputeSrvRootParameters, int nGraphicsSrvGpuHandles, int nComputeUavGpuHandles, int nComputeSrvGpuHandles)
{
	...
	if (m_nGraphicsSrvGpuHandles > 0)
	{
		...
		for (int i = 0; i < m_nGraphicsSrvGpuHandles; i++) m_pd3dGraphicsSrvGpuDescriptorHandles[i].ptr = NULL;
	}
	...
	if (m_nComputeUavGpuHandles > 0)
	{
		...
		for (int i = 0; i < m_nComputeUavGpuHandles; i++) m_pd3dComputeUavGpuDescriptorHandles[i].ptr = NULL;
	}
	...
	if (m_nComputeSrvGpuHandles > 0)
	{
		...
		for (int i = 0; i < m_nComputeSrvGpuHandles; i++) m_pd3dComputeSrvGpuDescriptorHandles[i].ptr = NULL;
	}
	...
	if (m_nGraphicsSrvRootParameters > 0)
	{
		...
		for (int i = 0; i < m_nGraphicsSrvRootParameters; i++) m_pnGraphicsSrvRootParameterDescriptors[i] = -1;
		m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles = new D3D12_GPU_DESCRIPTOR_HANDLE[m_nGraphicsSrvRootParameters];
		for (int i = 0; i < m_nGraphicsSrvRootParameters; i++) m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles[i].ptr = NULL;
	}
...
CTexture::~CTexture()
{
	...
	if (m_pnGraphicsSrvRootParameterDescriptors) delete[] m_pnGraphicsSrvRootParameterDescriptors;
	if (m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles) delete[] m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles;

	if (m_pnComputeUavRootParameterIndices) delete[] m_pnComputeUavRootParameterIndices;
	if (m_pnComputeUavRootParameteDescriptors) delete[] m_pnComputeUavRootParameteDescriptors;
	if (m_pd3dComputeUavRootParameterGpuDescriptorHandles) delete[] m_pd3dComputeUavRootParameterGpuDescriptorHandles;

	if (m_pnComputeSrvRootParameterIndices) delete[] m_pnComputeSrvRootParameterIndices;
	if (m_pnComputeSrvRootParameterDescriptors) delete[] m_pnComputeSrvRootParameterDescriptors;
	if (m_pd3dComputeSrvRootParameterGpuDescriptorHandles) delete[] m_pd3dComputeSrvRootParameterGpuDescriptorHandles;

	if (m_pd3dGraphicsSrvGpuDescriptorHandles) delete[] m_pd3dGraphicsSrvGpuDescriptorHandles;
	if (m_pd3dComputeSrvGpuDescriptorHandles) delete[] m_pd3dComputeSrvGpuDescriptorHandles;
	if (m_pd3dComputeUavGpuDescriptorHandles) delete[] m_pd3dComputeUavGpuDescriptorHandles;
...
{
	if ((m_pnComputeUavRootParameterIndices[nIndex] != -1) && (m_pd3dComputeUavRootParameterGpuDescriptorHandles[nIndex].ptr != NULL)) pd3dCommandList->SetComputeRootDescriptorTable(m_pnComputeUavRootParameterIndices[nIndex], m_pd3dComputeUavRootParameterGpuDescriptorHandles[nIndex]);
}

void CTexture::UpdateComputeSrvShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, int nIndex)
{
	if ((m_pnComputeSrvRootParameterIndices[nIndex] != -1) && (m_pd3dComputeSrvRootParameterGpuDescriptorHandles[nIndex].ptr != NULL)) pd3dCommandList->SetComputeRootDescriptorTable(m_pnComputeSrvRootParameterIndices[nIndex], m_pd3dComputeSrvRootParameterGpuDescriptorHandles[nIndex]);
}

void CTexture::UpdateGraphicsSrvShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, int nIndex)
{
	if ((m_pnGraphicsSrvRootParameterIndices[nIndex] != -1) && (m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles[nIndex].ptr != NULL)) pd3dCommandList->SetGraphicsRootDescriptorTable(m_pnGraphicsSrvRootParameterIndices[nIndex], m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles[nIndex]);
}
...
void CTexture::UpdateGraphicsShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	{
		if ((m_pnGraphicsSrvRootParameterIndices[i] != -1) && (m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles[i].ptr != NULL)) pd3dCommandList->SetGraphicsRootDescriptorTable(m_pnGraphicsSrvRootParameterIndices[i], m_pd3dGraphicsSrvRootParameterGpuDescriptorHandles[i]);
	}
}
...
void CMaterial::SetShader(CGraphicsShader *pShader)
{
	if (m_pShader) m_pShader->Release();
	m_pShader = pShader;
...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (m_pMaterial)
	{
		...
		{
			m_pMaterial->m_pShader->Render(pd3dCommandList, pCamera, NULL);
